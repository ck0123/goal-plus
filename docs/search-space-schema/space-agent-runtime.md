...
`scope` 应说明与本轮机制有关的 base 前提，`expected_new_information` 应明确单独效应还是组合
交互问题。例如“在 A 已存在的 hot loop 上加入 B，验证 A×B 是否改变 critical path”比只写
“尝试 B”更可判别。但 candidate 不需要也不能提交 artifact hash 或累积 diff。

收到 PlanCard 后，runtime 从实际 candidate workspace 构造内部的 authoritative
`BaseEvidence`：当前 artifact hash、从 workspace 初始 revision 到当前 artifact 的累积 delta
hash、changed files/symbols，以及 bounded diff stat/excerpt；同时记录当前 Git head 和最近
score。即使旧的内部调用者传入同名字段，runtime 也会覆盖它们。这个内部 envelope 不改变
MCP 的三字段 PlanCard schema，也不会返回给 candidate。

`BaseEvidence` 是语义投影的证据，不是 relevant base 本身。Git head 和 artifact hash 只能
证明完全相同的 revision/content 并提供 provenance；hash 不同不能证明实验新颖。SpaceAgent
必须结合当前 PlanCard、累积 diff 和历史 Evidence，从中临时投影出与本轮机制和问题相关的
base predicates。
...
- candidate 提交的 declared proposal，以及准入时 runtime 记录的 pre-intervention BaseEvidence；
...
**Evidence-anchored semantic coverage map**。判重单元不是 intervention 文本或 artifact
hash，而是一条条件实验边：
...
RelevantBaseProjection =
  project(current BaseEvidence | InterventionDelta, InformationTarget)
```

`RelevantBaseProjection` 只保留会影响当前 intervention 或 information target 的既有 artifact
条件；`InterventionDelta` 是准备新增的 material change；`InformationTarget` 是这个条件实验
实际回答的不确定性。projection 由 SpaceAgent 在审核时语义推导，不是 runtime 预先按 hash
划分，也不是新增的持久化 `PlanProjection`。所谓“区域”，是 Schema 对若干 Evidence-anchored
实验边的语义分组。
...
```text
admission mapping
  candidate PlanCard + runtime-authored current BaseEvidence
    -> SpaceAgent derives RelevantBaseProjection for this plan/question
    -> 对照 Schema coverage + raw Evidence tail + active reservations
    -> accept，或 reject(duplicate_of=[具体 plan ids])

evidence mapping
  pre-intervention BaseEvidence + declared PlanCard + realized artifact delta/result
...
verifier 结果优先于宽泛的 declared wording。Schema consolidation 必须在 coverage 的
description/context 中保留会改变实验身份的 base 前提和交互问题，不能把 `B-alone` 和
`B | A-present` 压成一个 cell。

Hash 在这里可以作为 exact-match 快路径或审计引用，但不能作为语义坐标。无关 comment 导致
两个 artifact hash 不同，如果两边都投影为同一个 `A-present` 前提，仍是相同 base projection；
反过来，SpaceAgent 也不能仅从 hash 或 symbol 名推断 A 仍然生效，而要结合最终累积 diff、
declared semantics 和 verifier-backed history 判断 A 是否存在、是否仍 material、是否与 B
相关。不确定时遵循 accept 策略。

映射的生命周期如下：

1. PlanCard 刚提交时只是待审核实验边；accept 后成为 active reservation，还不是已探索事实。
2. Verifier 完成后，它成为 tail 中的 raw Evidence edge，立刻参与后续判重。
...
```text
c001: clean -> A                           已覆盖 A-alone
c002: clean -> B                           已覆盖 B-alone
c001: A -> A+B, question=A×B interaction  新的条件实验，accept
later: A -> A+B, same A×B question         同一条件实验的改写，reject
```

如果 `A -> A+B` 与 `B -> A+B` 最终形成 material 等价的组合，并回答同一个 A×B 问题，操作
顺序不同也不产生两条实验边。反过来，base 里存在一个与机制无关的改动，也不会仅凭 artifact hash
不同获得新颖性。
...
+ candidate PlanCard + authoritative current BaseEvidence
```
...
- 尚未归并的 compact Evidence event views，包括每次实验的 pre-intervention BaseEvidence、
  declared PlanCard、realized delta 和 base/result revision；
- active reservation 的 plan id、candidate id、PlanCard 和 pre-intervention BaseEvidence。

因此一次判定拥有 run 级全局视野，但不是把所有原始文件重放一遍：已归并历史由当前 Schema
coverage 表示，最近最多一个 checkpoint 窗口的事实由 Evidence tail 表示，尚未完成的并发
工作由 reservations 表示；当前 lane 的 artifact 原始证据由 runtime-authored BaseEvidence
表示，SpaceAgent 再针对当前问题得到 RelevantBaseProjection。
...
```text
candidate PlanCard + current BaseEvidence
+ Search Schema + Search Evidence tail + reservations
```
...
- pre-intervention base/result Git refs 和 artifact hash；
- BaseEvidence 的累积 delta hash、最多 12 个 changed files/symbols、最多 400 字符 diff stat、
  最多 1,000 字符 diff excerpt；
- artifact delta hash；
...
磁盘中每个 BaseEvidence 的累积 diff excerpt 最多 4,000 字符、diff stat 最多 1,000 字符，
changed files/symbols 最多保留 64 个、合计 8,000 字符，同时保存截断前 count；admission view
会再次收紧到上述 12 个/1,200 字符和 1,000/400 字符。PlanCard 每个字段最多 2,000 字符。普通
admission 中，每个 coverage cell 最多带 4 个代表性
...
- 实验身份按 `<RelevantBaseProjection, InterventionDelta, InformationTarget>` 判断，不能只比较
  intervention，也不能把 base hash 直接当成 RelevantBaseProjection；
- 普通 duplicate 只有在以下三项同时成立时才能 reject：relevant base conditions materially
  equivalent、intervention deltas materially equivalent、information target already covered；
- 另一条 reject 路径是：已有 Evidence 已经蕴含同一个 verified target state，并且新 edge
  没有提供额外的 marginal-effect 或 order information；
- 文字改写但 material intervention 和信息目标相同，reject；
- 只把 broad plan 说得更详细，不自动成为新实验；
- `B-alone` 已做过，不代表 `B-on-A` 必须 reject；当后者明确回答尚未覆盖的 A×B 交互问题时
  可以 accept；
- base、参数、上下文或机制存在与问题相关的实质差异，并会回答新的问题，可以 accept；
- 不同的 Git head/artifact hash 不能证明新颖；相同 score 也不能证明 base state 等价；
- 不同 relevant base 只有在改变 mechanism、conditional effect、target state 或获得的信息时，
  才构成应该 accept 的新条件实验；
- `A -> A+B` 与 `B -> A+B` 的 material composition 和交互问题等价、且不新增 order
  information 时，视为重复；
- refinement、replication、interaction、alternative implementation 这些标签本身不能证明
  新颖；
- relevance 或 target-state equivalence 不确定时 accept；
...
这里的“等价”包含条件组合等价：一个 lane 在 A base 上申请 B，另一个 lane 在 B base 上
申请 A；如果两者都形成同一个 A+B composition 并回答同一交互问题，CAS 后只保留一个
reservation。若前者是 A×B interaction、后者只是 A-alone 的独立效应，则不是同一实验边。
...
回退后的下一次 admission 会重新从当前 workspace 构造 BaseEvidence，不会谎称已撤销的 A
仍存在。但回退不删除旧 Event：如果先前已经有效完成 `A -> A+B` 的 A×B 实验，回到 A 后
再次申请同一 `B-on-A` 仍会命中旧 coverage；回到 clean base 后申请 `B-alone` 是否重复，则
只与已覆盖的 clean-base B 实验比较。SpaceAgent 区分 artifact 当前状态与历史事实。
