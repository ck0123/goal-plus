...
1. `Task` 工具创建一个 child session。
2. foreground task 下，父会话等待 child session 完成；如果父会话被 abort，会 cancel child session。
...
- foreground `Task` 在父级 abort 时会 abort child task context，并 cancel child session。
- `SessionRunState.cancel` 会 interrupt 当前 runner。
...
这个模型类似“等待完成或超时后把控制权还给上层”的 supervisor。但 Search MCP 需要额外补上三件事：
