...
strategy decides candidate directions
  -> main agent starts managed agent sessions
...
这不是一次短任务派发，而是一个可持续观察、可等待、可中断的 search session。
...
`start_agent_session` 表达的是启动一条自主搜索链，而不是把候选人一次性派给一个不可观察的长跑任务。
...
旧 worker dispatch API 已删除，避免 host agent 再误走 foreground Task + 软 deadline 的旧流程。新控制面只保留 agent session 语义；旧 spec 字符串在模型解析时归一到 `agent-session-pool`。

### Step 1: 引入 AgentSession
...
3. 新版 OpenCode 在 `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true`（或 `OPENCODE_EXPERIMENTAL=true`）下暴露 `Task(background=true)`；这时 Task 调用会立即返回 running task，父 agent 可以继续执行 wait/status/abort 逻辑。
4. 每个 agent 可以配置 `steps`，旧字段 `maxSteps` 只是兼容入口。
5. runner 每一轮维护 `step` 计数；当 `step >= agent.steps` 时，不再 materialize tools，设置 `toolChoice: none`，并额外塞入 `MAXIMUM STEPS REACHED` 的强制总结 prompt。
6. 如果模型在 final step 仍然发出 tool call，runner 会把未完成 tool 标记为失败，原因是 tools 已因最大步数被禁用。
...
- `Task` schema 没有通用 `timeout` 参数。`worker_timeout_seconds` 不能被直接映射成 OpenCode Task 参数。
- `Task(background=true)` 被实验开关隐藏；必须在启动 OpenCode 父进程时设置 `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true`，放进 MCP server 的环境变量没有用。
...
OpenCode 支持 background subagent，使 main agent 不再被 foreground Task 阻塞。
...
5. OpenCode host 必须用 `Task(background=true)`，并在启动 OpenCode 父进程时设置 `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true`。
6. 如果 host 以后接入 OpenCode session control，再把 hard deadline 映射成真实 `cancel(child_session_id)`。
