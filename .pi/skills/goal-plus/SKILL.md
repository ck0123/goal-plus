...
`goal_plus_list_models(host="pi-rpc")`，将用户填写的名称解析为唯一可用模型并冻结到
`strategy.models`；不存在或不唯一时，在创建
...
不要在 SearchSpec 中生成 `acceptance_view`、软 rubric 或预设评价维度。Spec Discovery
只能冻结硬 metric、verifier、编辑范围、预算和 promotion 合同；新 run 若包含旧式
