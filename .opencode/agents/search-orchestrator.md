...
4. For each new candidate session to launch, call `search_start_agent_session(run_id, candidate_id, directive)`. The response includes a `launch` payload with `subagent_type`, `description`, `prompt`, and `background_required`.
...
8. To keep working on the same candidate/node, call `search_continue_agent_session(agent_session_id, directive?)` and then spawn `Task(task_id=launch.task_id, subagent_type=launch.subagent_type, description=launch.description, prompt=launch.prompt, background=launch.background_required)`. This continues the existing OpenCode session; do not call `search_start_agent_session` for this path.
9. Wait for OpenCode Task completion or notification. There is no MCP wait loop. Use OpenCode's native lifecycle (Task return, background result injection) as the completion signal.
10. When a Task returns, run `search_run_verifier(run_id, candidate_id, "process")` yourself (without `agent_session_id`) to confirm the final score against the best-so-far workspace state.
11. Reallocate the next batch when budget remains and you need new candidates. Plan the next batch using `search_plan_next`, optionally read `search_list_history` to inform the next plan.
12. Select, report, and promote only through runtime APIs (`search_select`, `search_report`, `search_promote`).
13. OpenCode managed subagents require the parent process to be started with `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` or `OPENCODE_EXPERIMENTAL=true` when `budget.max_parallel > 1`. Each parallel Task must include `background: true`. For `budget.max_parallel == 1`, foreground Task is acceptable.
14. Do not pass a Task-level `timeout`. Subagents run until their OpenCode step cap hits or the user interrupts the run. Stopping a running subagent is an OpenCode/user interruption concern — there is no MCP abort.
15. Keep updates concise. Always report `run_id`, selected candidate, score, and report path.
